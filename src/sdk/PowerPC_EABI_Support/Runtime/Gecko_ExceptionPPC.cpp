// https://github.com/SwareJonge/mkdd/blob/main/libs/PowerPC_EABI_Support/src/Runtime/Gecko_ExceptionPPC.cp
...
static exaction_type ExPPC_NextAction(ActionIterator *iter) {
    ...
    for (;;) {
            ...
            switch (action) {
                ...
                case EXACTION_SPECIFICATION:
                    iter->info.action_pointer += sizeof(ex_specification)
                        + ((ex_specification *)iter->info.action_pointer)->specs
                            * sizeof(void *);
                    break;
                default:
                    std::terminate();
        ...
        if (action == EXACTION_BRANCH) {
            ...
            action =
                ((ex_destroylocal *)iter->info.action_pointer)->action & EXACTION_MASK;
        }
        return action;
    }
}
    ...
    for (;;) {
        ...
        switch (action & EXACTION_MASK) {
            ...
            case EXACTION_DESTROYLOCALCOND:
                ...
                break;
            case EXACTION_DESTROYLOCALPOINTER:
                ExPPC_DestroyLocalPointer(
                    context, (ex_destroylocalpointer *)info->action_pointer
                );
            ...
            case EXACTION_DESTROYMEMBERCOND:
                ...
                break;
            case EXACTION_DESTROYMEMBERARRAY:
                ExPPC_DestroyMemberArray(
                    context, (ex_destroymemberarray *)info->action_pointer
                );
            ...
            case EXACTION_SPECIFICATION:
                if (catcher == (void *)info->action_pointer)
                    return;
                info->action_pointer += sizeof(ex_specification)
...
static inline void ExPPC_HandleUnexpected(
    ThrowContext *context, MWExceptionInfo *info, ex_specification *unexp
) {
    CatchInfo *catchinfo;
...
static void ExPPC_ThrowHandler(ThrowContext *context) {
    ...
    for (action = ExPPC_CurrentAction(&iter);; action = ExPPC_NextAction(&iter)) {
        switch (action) {
            ...
            case EXACTION_CATCHBLOCK:
                ...
                continue;
            case EXACTION_SPECIFICATION:
                if (!ExPPC_IsInSpecification(
                        context->throwtype, (ex_specification *)iter.info.action_pointer
                    )) {
                    ExPPC_HandleUnexpected(
                        context, &info, (ex_specification *)iter.info.action_pointer
                    );
                }
                continue;
            case EXACTION_ENDOFLIST:
    ...
    if (action == EXACTION_CATCHBLOCK_32) {
        ...
        ExPPC_LongJump(
            context, info.TOC, info.current_function + catchblock_32->catch_pcoffset
        );
    } else {
        ...
        ExPPC_LongJump(
            context, info.TOC, info.current_function + catchblock->catch_pcoffset
        );
    }
}
